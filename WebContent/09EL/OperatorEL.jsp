...
<body>
	...
	*/
	...
	<!-- 
		...
		만약 해당페이지?myNumber=20 인 경우 30 출력됨
		해당페이지 ?myNumber= 인 경우 0으로 간주되어 10 출력됨
		해당페이지 ?myNumber=삼 인 경우 문자는 숫자로 변경불가하므로 에러 발생됨.
		double인 경우는 에러남
	 -->
	...
	<%
	/* 
	EL에서는 JSP에서 선언한 변수는 직접 사용할 수 없다. 값은 출력되지않고
	null로 인식하게 된다.
	...
	*/
	...
	<!-- 
		...
		하지만 개발시에는 실제 서비스할 웹서버의 버전을 확인 후 사용여부를 결정한다.
	...
	\${fnum=99 } : ${fnum=99 } <!-- 99로 재할당 되어 99 출력됨 -->
	
	<!-- 
		EL에서는 정수와 정수를 연산하더라도 '실수'의 결과가
		나올 수 있따. 즉 자동형변환되어 출력된다.
	...
	<ul>
		...
		<!-- 변수형태의 숫자의경우 숫자가 아닌 문자형태로 들어가있기 때문에 문자열 비교가 됨 -->
		<li>\${100 > 90 } : ${100 > 90 }</li><!-- 결과 : true -->
		
		<!-- 
			Java에서는 문자열을 비교할 떄 equals()메소드를 통해 비교하지만
			EL에서는 ==형태로 비교한다.
		 -->
